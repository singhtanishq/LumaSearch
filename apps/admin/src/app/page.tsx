  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <span
          className={`w-3 h-3 rounded-full ${health.status === 'ok' ? 'bg-green-500' : health.status === 'degraded' ? 'bg-amber-500' : 'bg-red-500'}`}
        />
        ...
        <span className="text-sm text-gray-500">
          Uptime: {Math.floor(health.uptimeSeconds / 60)}m
        </span>
      </div>
        ...
        {Object.entries(health.checks).map(([name, check]) => (
          <div key={name} className="p-4 border border-gray-200 dark:border-gray-800 rounded-lg">
            ...
            <p className="text-sm text-gray-500 mt-1">
              {check.latencyMs ? `${check.latencyMs}ms` : 'N/A'}
            </p>
    ...
    <div className="space-y-4">
      ...
      {jobs.length === 0 ? (
        <div className="text-center py-12 text-gray-500">
          No crawl jobs yet. Submit one via API.
        </div>
      ) : (
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-gray-500">
        ...
        <code className="bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">{API_URL}/metrics</code>.
