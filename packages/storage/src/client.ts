...
export function createPrismaClient(options: { url?: string; log?: ('query' | 'info' | 'warn' | 'error')[] } = {}): PrismaClient {
...
export function getPrismaClient(options: { url?: string; log?: ('query' | 'info' | 'warn' | 'error')[] } = {}): PrismaClient {
...
export async function checkDbHealth(prisma: PrismaClient): Promise<{ status: 'ok' | 'down'; latencyMs: number }> {
